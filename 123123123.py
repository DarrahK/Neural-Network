...
class Network:
    ...
    def feedForward(self, a_data):
        ...
        for x in range(self.layers - 1):
            self.zs.append(np.add(np.matmul(self.weights[x], self.nodes[x]), self.biases[x]))
            self.nodes.append(sigmoid(np.add(np.matmul(self.weights[x], self.nodes[x]), self.biases[x])))
    ...
    def back_prop(self, x, y):
        ...
        for back in range(1, self.layers - 1):
            ...
            delta = np.multiply(np.dot(weight.transpose(),delta), sigmoid_prime(self.zs[-back-1]))
...
def cost_prime(self, y):
    return 2*np.subtract(self.nodes[-1], y)

A = Network([2,3,1])
B, C = A.back_prop([1,2],[1])
